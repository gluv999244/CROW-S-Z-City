...
local song
local songfade = 0
net.Receive("cinvasion_start", function()
	surface.PlaySound("snd_jack_hmcd_disaster.mp3") 

	timer.Simple(3, function()
		sound.PlayFile( "sound/hl2mode1.wav", "mono noblock", function( station )
...
local teams = {
	[0] = {
		objective = "Copy that, Sending in soldiers to take over.",
		name = "a Combine Soldier",
		color1 = Color(10, 10, 255),
		color2 = Color(10, 10, 255)
	},
	[1] = {
		objective = "Some weird object is floating above us...is it a UFO?",
		name = "a Bystander",
...
function MODE:RenderScreenspaceEffects()
	...
	if zb.ROUND_START + 85 < CurTime() then
		...
		if songfade <= 0.01 and IsValid( song ) then
			song:Stop()
			if lply:Team() == 0 then
				surface.PlaySound("zbattle/criresp/barricadedsuspectstart.mp3")
			end
...
function MODE:HUDPaint()
	if zb.ROUND_START + 90 > CurTime() then
		...
		draw.SimpleText( "The Combine will arrive in: "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText( "The Combine will arrive in: "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
		surface.SetDrawColor(0, 0, 0, 255 * fade)
		surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
	end
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("Combine Invasion", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local CreateEndMenu
net.Receive("ci_roundend", function() CreateEndMenu(net.ReadBool()) end)
